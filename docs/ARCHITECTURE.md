...
1. **Memory Usage:** Heuristic pipeline keeps full dataset in memory (7.5M records requires ~32GB RAM for optimal performance).
2. **Confidence Scores:** Heuristic confidence scores are rule-based approximations, not true probabilities.
3. **DuckDB Integration:** Splink pipeline uses embedded DuckDB which is efficient but shares resources with the Python process.

## Pipelines

The system offers two distinct pipelines for duplicate detection:

### 1. Heuristic Pipeline (`run_optimized_analysis.py`)
- **Philosophy:** Speed and transparency. Uses deterministic blocking rules + fuzzy matching.
- **Blocking:** Multi-pass strategy (Address Pass -> Phonetic Pass).
- **Matching:** `rapidfuzz` (Levenshtein/Jaro-Winkler) with vectorized preprocessing.
- **Output:** Simple CSV with `match_id` and confidence score.
- **Best for:** Operational runs, daily checks, full dataset processing (7.5M records).

### 2. Probabilistic Pipeline (`dedupe_splink/`)
- **Philosophy:** Statistical accuracy. Uses Fellegi-Sunter model via Splink v4.
- **Blocking:** Probabilistic blocking rules generated by training.
- **Matching:** EM algorithm estimates m/u probabilities for each field.
- **Output:** Edge list with match probabilities and clustered components (Graph).
- **Best for:** Complex cases, discovering non-obvious duplicates, training weights.
...
### Current State
- **Deployment Type:** CLI execution
- **Execution Model:** Python scripts (`run_optimized_analysis.py`, `run_splink_end2end.py`)
